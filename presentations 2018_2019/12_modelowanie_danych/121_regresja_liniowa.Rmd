...
```{r}
svytotal(x = ~api.stu, design = schemat)
```
...
```{r}
r1 <- svyratio(numerator = ~api.stu, denominator = ~enroll, design = schemat)

predict(r1, total = sum(apipop$enroll, na.rm = T))

```

```{r}
ggplot(apistrat, aes(enroll, api.stu)) + 
  geom_point() +
...
```{r}
svytotal(x = ~ell, schemat)
```

```{r}
r2 <- svyratio(numerator = ~ell, denominator = ~enroll, schemat)
r2
```

```{r}
predict(r2, total = sum(apipop$enroll, na.rm = T))
```

```{r}
ggplot(apistrat, aes(ell, api.stu)) + 
  geom_point() +
  geom_abline(intercept = 0)
...
```{r}
dane <- data.frame(enroll=sum(apipop$enroll, na.rm = T))
...
```{r}
predict(m3, newdata = dane, total = nrow(apipop))
```

```{r}
ggplot(apistrat, aes(enroll, ell)) + 
  geom_point() +
  geom_smooth(method="lm")
```