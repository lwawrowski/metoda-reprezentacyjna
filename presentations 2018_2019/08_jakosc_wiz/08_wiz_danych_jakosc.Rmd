...
```{r wykres słupkowy}
ggplot(region, aes(x=region, y=one)) + 
  geom_col() +
...
region_sex <- svyby(formula = ~one, by = ~region+sex, design = schemat, FUN = svytotal)
```

```{r 1 wykres słupkowy region i płeć}
ggplot(region_sex, aes(x=region, y=one, fill=sex)) + 
  geom_col() +
  coord_flip()
```
...
```{r wykres kafelkowy}
ggplot(region_sex, aes(x=sex, y=region, fill=one)) + 
  geom_tile() +
  geom_text(aes(label=one), color="white") +
